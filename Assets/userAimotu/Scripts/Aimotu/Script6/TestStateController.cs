...
/*
    ...
    {
        var gm = FindObjectOfType<MonoBehaviour>() as IGameManager;
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Alpha1))
                Switch(RoomState.Intro, false);

            // 数字键 2: 到 NoteLocked (便利贴锁定)
            if (Input.GetKeyDown(KeyCode.Alpha2))
                Switch(RoomState.NoteLocked, false);

            // 数字键 3: 到 PasswordCollecting (可搜集密码)
            if (Input.GetKeyDown(KeyCode.Alpha3))
                Switch(RoomState.PasswordCollecting, false);

            // 数字键 4: 到 AllTasksDone (任务全满，可点床)
            if (Input.GetKeyDown(KeyCode.Alpha4))
                Switch(RoomState.AllTasksDone, true);

            // 数字键 5: 到 ReadyToExit (准备切场)
            if (Input.GetKeyDown(KeyCode.Alpha5))
                Switch(RoomState.ReadyToExit, true);
        }
